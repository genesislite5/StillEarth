...
class NPC3:
    def __init__(self, x, y, game, chat_bubble_manager, npc_id):
        self.x = x
        self.y = y

        self.game = game
        self.chat_bubble_manager = chat_bubble_manager
        self.id = npc_id  # Unique ID for the NPC
        
        # Set the target dimensions for both idle and run animations
        self.target_width = 72  # width of the idle animation
        self.target_height = 108  # height of both animations
        self.width = self.target_width
        self.height = self.target_height
        self.collision_cooldown = 0  # Add this line
        self.idle_sprites = self.load_spritesheet("NPC3/idle/pixil-frame-0-93.png", 12, 18, 6, self.target_width, self.target_height)
        self.run_sprites = self.load_spritesheet("NPC3/run/pixil-frame-0-94.png", 12, 18, 4, self.target_width, self.target_height)
        ...
        self.last_direction = 1  # 1 for right, -1 for left
        self.dialogue_timer = time.time()  # Timer for dialogue
    ...
    def load_spritesheet(self, path, frame_width, frame_height, frame_count, target_width, target_height):
        ...
        for i in range(frame_count):
            ...
            # Scale the frame to the target width and height
    ...
    def update(self, dt, camera_x, camera_y):
        ...
        # Handle chat bubble timing
        if time.time() - self.dialogue_timer > random.uniform(5, 15):  # Random time between 5 and 15 seconds
    ...
    def show_random_dialogue(self):
        dialogues = ["we have to save StillEarth", "ugh there are boxes everywhere"]
        dialogue = random.choice(dialogues)
        self.chat_bubble_manager.add_bubble(self.id, dialogue, (self.x, self.y))
