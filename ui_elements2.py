...
def load_overlay_images3():
    boxes_image = pygame.image.load('data/overlay_images/boxes.png').convert_alpha()
    boxeswhite_image = pygame.image.load('data/overlay_images/boxes_white.png').convert_alpha()
    return boxes_image, boxeswhite_image

def load_toolbar_item3(image_path):
...
def load_overlay_position3():
    try:
        with open('overlay_position3.json', 'r') as f:
...
def render_overlay3(screen, current_overlay_image, overlay_pos, camera_x, camera_y):
    if overlay_pos:
        screen.blit(current_overlay_image, (int(overlay_pos[0] - camera_x), int(overlay_pos[1] - camera_y)))

def render_toolbar3(screen, item_image, item_slot, is_selected):
...
def add_item_to_toolbar(toolbar_items, image_path):
    item_image, item_size, item_id = load_toolbar_item3(image_path)
