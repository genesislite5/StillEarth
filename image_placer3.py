...
class ImagePlacer:
    ...
    def save_position(self):
        if self.overlay_pos:
            with open('overlay_position3.json', 'w') as f:
                json.dump(list(self.overlay_pos), f)
            print("Overlay position saved.")

    def delete_position(self):
        self.overlay_pos = None
        if os.path.exists('overlay_position3.json'):
            os.remove('overlay_position3.json')
...
def main():
    ...
    image_placer = ImagePlacer('/Users/toneanmorgan/Downloads/boxes.png', background_paths)
