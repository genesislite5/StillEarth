...
def load_overlay_images():
    nowhite_image = pygame.image.load('data/images/nowhite.png').convert_alpha()
    yeswhite_image = pygame.image.load('data/images/yeswhite.png').convert_alpha()
    return nowhite_image, yeswhite_image

def load_toolbar_item(image_path):
...
def render_overlay(screen, current_overlay_image, overlay_pos, camera_x, camera_y):
    if overlay_pos:
        screen.blit(current_overlay_image, (int(overlay_pos[0] - camera_x), int(overlay_pos[1] - camera_y)))

def render_toolbar(screen, item_image, item_slot, is_selected):
...
def add_item_to_toolbar(toolbar_items, image_path):
    item_image, item_size, item_id = load_toolbar_item(image_path)
...
def main():
    ...
    nowhite_image, yeswhite_image = load_overlay_images()
    overlay_pos = load_overlay_position()
    ...
    while running:
        ...
        render_overlay(screen, nowhite_image, overlay_pos, 0, 0)
        for index, item in enumerate(toolbar_items):
            render_toolbar(screen, item['image'], item['rect'], False)
